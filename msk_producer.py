#!/usr/bin/env python3
"""
AWS MSK (Managed Streaming for Apache Kafka) Producer 示例
支持 SASL_SSL 和 PLAINTEXT 安全协议
"""
        ...
        if self.config.security_protocol == "SASL_SSL":
            ...
            # AWS MSK IAM 认证
                ...
                try:
                    ...
                    config['sasl_oauth_token_provider'] = MSKAuthTokenProvider(
                        region=self.config.region
                    )
                except ImportError:
                    self.logger.error("需要安装 aws-msk-iam-sasl-signer-python 包")
                    raise
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if metadata:
                ...
                self.logger.error("无法连接到 MSK 集群")
                return False
